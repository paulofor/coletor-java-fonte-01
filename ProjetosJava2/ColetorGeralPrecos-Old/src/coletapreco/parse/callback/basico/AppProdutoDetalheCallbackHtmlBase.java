...
import coletapreco.parse.dados.basico.AppProdutoDadosParseBase;
import coletapreco.modelo.AppProduto;
import coletapreco.log.ArquivoLog;


public abstract class AppProdutoDetalheCallbackHtmlBase  extends CallbackParseHtml{

	protected AppProdutoDadosParseBase dadosParse;
	...
	public final void setDados(IDadosParse paramIDadosParse) {
		dadosParse = (AppProdutoDadosParseBase) paramIDadosParse;
		ArquivoLog.getInstancia().salvaLog("AppProduto(detalhe):"  + dadosParse.displayLog(dadosParse.getItemDetalhe()));
	}
	...
	public final void finalizacaoOk() {
		//if (lista==null) throw new RuntimeException("objeto lista em AppProdutoListaCallbackHtmlBase esta null");
		finalizaObjeto();
	...
	private void limpaListasInternas() {
		...
		this.dadosParse.getItemDetalhe().setListaFacebookFanpage_DivulgadoPor(null);
 	
		this.dadosParse.getItemDetalhe().setListaNaturezaProduto_Atende(null);
 	
		this.dadosParse.getItemDetalhe().setListaDispositivoUsuario_UsadoPor(null);
