...
import coletapreco.parse.callback.PrecoDiarioListaCallbackHtml;
import coletapreco.parse.callback.PrecoDiarioDetalheCallbackHtml;
import coletapreco.parse.dados.PrecoDiarioDadosParse;
...
public abstract class PrecoDiarioRegraColecaoAdaptador  {
	...
	protected PrecoDiarioDadosParse dadosParse = null;
	
	public PrecoDiarioRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new PrecoDiarioDadosParse();
	}
	private ICallbackParse getCallbackLista() {
		...
		if (callbackLista==null) {
			System.out.println("PrecoDiario ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaListaCallbackHtml()");
		}
	...
	private ICallbackParse getCallbackDetalhe() {
		...
		if (callbackDetalhe==null) {
			System.out.println("PrecoDiario ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaDetalheCallbackHtml()");
		}
		return callbackDetalhe;
	}
	protected ICallbackParse criaListaCallbackHtml() {
		return new PrecoDiarioListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new PrecoDiarioDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(PrecoDiarioDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		PrecoDiarioDao daoFonte = DBB.getInstancia().getPrecoDiarioDao(); // 
		daoFonte.setConexao(conexao);
		List<PrecoDiario> lista = daoFonte.ListaCorrente();
		Iterator<PrecoDiario> itLista = lista.iterator();
		while (itLista.hasNext()) {
			PrecoDiario corrente = itLista.next();
	...
	public final void atualizaDetalhe(PrecoDiario itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(PrecoDiarioDadosParse dados) {
	}
	public PrecoDiario getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new PrecoDiarioListaCallbackHtml();
		PrecoDiarioDadosParse dadosParse = new PrecoDiarioDadosParse();
		dadosParse.setConexao(conexao);
