...
import coletapreco.parse.callback.AppProdutoListaCallbackHtml;
import coletapreco.parse.callback.AppProdutoDetalheCallbackHtml;
import coletapreco.parse.dados.AppProdutoDadosParse;
...
public abstract class AppProdutoRegraColecaoAdaptador  {
	...
	protected AppProdutoDadosParse dadosParse = null;
	
	public AppProdutoRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new AppProdutoDadosParse();
	}
	private ICallbackParse getCallbackLista() {
		...
		if (callbackLista==null) {
			System.out.println("AppProduto ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaListaCallbackHtml()");
		}
	...
	private ICallbackParse getCallbackDetalhe() {
		...
		if (callbackDetalhe==null) {
			System.out.println("AppProduto ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaDetalheCallbackHtml()");
		}
		return callbackDetalhe;
	}
	protected ICallbackParse criaListaCallbackHtml() {
		return new AppProdutoListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new AppProdutoDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(AppProdutoDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		AppProdutoDao daoFonte = DBB.getInstancia().getAppProdutoDao(); // 
		daoFonte.setConexao(conexao);
		List<AppProduto> lista = daoFonte.ListaCorrente();
		Iterator<AppProduto> itLista = lista.iterator();
		while (itLista.hasNext()) {
			AppProduto corrente = itLista.next();
	...
	public final void atualizaDetalhe(AppProduto itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(AppProdutoDadosParse dados) {
	}
	public AppProduto getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new AppProdutoListaCallbackHtml();
		AppProdutoDadosParse dadosParse = new AppProdutoDadosParse();
		dadosParse.setConexao(conexao);
