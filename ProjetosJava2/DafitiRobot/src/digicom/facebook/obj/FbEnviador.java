...
public class FbEnviador implements Job{
	...
	public void inicializaFacebook() {
		...
		String MY_ACCESS_TOKEN = "CAABwbur8GF4BAH8E2xf6tfegSZBc2ucH2ZCyKV1Ug3KzkZCYoEQ5VkYMfgZAS13X2RVKQPP3kTUYWa0dExMJeAC3MfQC2reEzbCEAHVjU0UzbsKZA1PZBfnnaGNrBoBabWvZCQqmwMWbpZAmZCqwmQRbhjcAX7mCWxxDPkE5Ufag1E9lY2PJ08igOt99LvNlPbFCrOsZAbPLeJb9QL3jUoXrjP";
		// Buscador 
	...
	public void EnviaOportunidades()  {
		OportunidadeDiaRegraColecao oportunidadeSrv = FabricaRegra.getInstancia().getOportunidadeDiaRegraColecao();
		oportunidadeSrv.getFiltro().setCodigoCategoriaProduto(ID_CATEGORIA);
		try {
			OportunidadeDia corrente = oportunidadeSrv.ObtemProximoPorCategoria();
			corrente.setCodigoFacebook(EnviaFotoPost(corrente));
			//corrente.setCodigoFacebook(EnviaFotoPost(corrente, FANPAGE_ID));
			//EnviaPost(corrente,FANPAGE_ID);
	...
	private String EnviaFotoPost(OportunidadeDia oportunidade) throws FileNotFoundException {
		...
		String mensagem = getMensagem(oportunidade);
		File imageFile = new File(ProdutoRegraColecaoImpl.diretorioDownload
