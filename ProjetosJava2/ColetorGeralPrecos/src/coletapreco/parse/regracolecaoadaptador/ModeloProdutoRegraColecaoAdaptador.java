...
import coletapreco.parse.callback.ModeloProdutoListaCallbackHtml;
import coletapreco.parse.callback.ModeloProdutoDetalheCallbackHtml;
import coletapreco.parse.dados.ModeloProdutoDadosParse;
...
public abstract class ModeloProdutoRegraColecaoAdaptador  {
	...
	protected ModeloProdutoDadosParse dadosParse = null;
	
	public ModeloProdutoRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new ModeloProdutoDadosParse();
	}
	private ICallbackParse getCallbackLista() {
		...
		if (callbackLista==null) {
			System.out.println("ModeloProduto ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaListaCallbackHtml()");
		}
	...
	private ICallbackParse getCallbackDetalhe() {
		...
		if (callbackDetalhe==null) {
			System.out.println("ModeloProduto ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaDetalheCallbackHtml()");
		}
		return callbackDetalhe;
	}
	protected ICallbackParse criaListaCallbackHtml() {
		return new ModeloProdutoListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new ModeloProdutoDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(ModeloProdutoDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		ModeloProdutoDao daoFonte = DBB.getInstancia().getModeloProdutoDao(); // 
		daoFonte.setConexao(conexao);
		List<ModeloProduto> lista = daoFonte.ListaCorrente();
		Iterator<ModeloProduto> itLista = lista.iterator();
		while (itLista.hasNext()) {
			ModeloProduto corrente = itLista.next();
	...
	public final void atualizaDetalhe(ModeloProduto itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(ModeloProdutoDadosParse dados) {
	}
	public ModeloProduto getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new ModeloProdutoListaCallbackHtml();
		ModeloProdutoDadosParse dadosParse = new ModeloProdutoDadosParse();
		dadosParse.setConexao(conexao);
