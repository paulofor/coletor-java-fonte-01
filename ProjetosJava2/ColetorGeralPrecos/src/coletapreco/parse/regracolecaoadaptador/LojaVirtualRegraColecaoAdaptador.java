...
import coletapreco.parse.callback.LojaVirtualListaCallbackHtml;
...
public abstract class LojaVirtualRegraColecaoAdaptador  {
	...
	protected LojaVirtualDadosParse dadosParse = null;
	
	public LojaVirtualRegraColecaoAdaptador() {
		exec = new ExecutadorParse();
		dadosParse = new LojaVirtualDadosParse();
	}
	private ICallbackParse getCallbackLista() {
		...
		if (callbackLista==null) {
			System.out.println("LojaVirtual ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaListaCallbackHtml()");
		}
	...
	private ICallbackParse getCallbackDetalhe() {
		...
		if (callbackDetalhe==null) {
			System.out.println("LojaVirtual ID:" + this.getItemRaiz().getIdObj() + " nao possui Callback em " + this.getClass().getSimpleName() + ".criaDetalheCallbackHtml()");
		}
		return callbackDetalhe;
	}
	protected ICallbackParse criaListaCallbackHtml() {
		return new LojaVirtualListaCallbackHtml();
	}
	protected ICallbackParse criaDetalheCallbackHtml() {
		return new LojaVirtualDetalheCallbackHtml();
	}
	...
	protected void preparaDadosLista(LojaVirtualDadosParse dados) {
	}
	...
	public void atualizaDetalhe(DaoConexao conexao) throws DaoException {
		LojaVirtualDao daoFonte = DBB.getInstancia().getLojaVirtualDao(); // 
		daoFonte.setConexao(conexao);
		List<LojaVirtual> lista = daoFonte.ListaCorrente();
		Iterator<LojaVirtual> itLista = lista.iterator();
		while (itLista.hasNext()) {
			LojaVirtual corrente = itLista.next();
	...
	public final void atualizaDetalhe(LojaVirtual itemRaiz, DaoConexao conexao) throws DaoException {
	...
	protected void preparaDadosDetalhe(LojaVirtualDadosParse dados) {
	}
	public LojaVirtual getItemRaiz() {
	...
	private void atualizaItens(DaoConexao conexao) throws DaoException {
		...
		ICallbackParse callback = new LojaVirtualListaCallbackHtml();
		LojaVirtualDadosParse dadosParse = new LojaVirtualDadosParse();
		dadosParse.setConexao(conexao);
