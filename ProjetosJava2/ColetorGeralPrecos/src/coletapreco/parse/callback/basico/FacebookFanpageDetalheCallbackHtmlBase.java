...
import coletapreco.parse.dados.basico.FacebookFanpageDadosParseBase;
import coletapreco.modelo.FacebookFanpage;
import coletapreco.log.ArquivoLog;


public abstract class FacebookFanpageDetalheCallbackHtmlBase  extends CallbackParseHtml{

	protected FacebookFanpageDadosParseBase dadosParse;
	...
	public final void setDados(IDadosParse paramIDadosParse) {
		dadosParse = (FacebookFanpageDadosParseBase) paramIDadosParse;
		ArquivoLog.getInstancia().salvaLog("FacebookFanpage(detalhe):"  + dadosParse.displayLog(dadosParse.getItemDetalhe()));
	}
	...
	public final void finalizacaoOk() {
		//if (lista==null) throw new RuntimeException("objeto lista em FacebookFanpageListaCallbackHtmlBase esta null");
		finalizaObjeto();
	...
	private void limpaListasInternas() {
		...
		this.dadosParse.getItemDetalhe().setListaFacebookPost_Tem(null);
