...
public class LojaVirtualRicardoDetalheCallbackHtml extends LojaVirtualDetalheCallbackHtml {
	...
	public LojaVirtualRicardoDetalheCallbackHtml() {
		//this.setDebug();
	}
	...
	protected void inicioTag(Tag t, String classeNome, String idNome) {
		...
		if (t == HTML.Tag.DIV && "info".equals(classeNome)) {
			ligaColeta = true;
		}
	...
	public void handleText(char[] data, int pos) {
		...
		if (ligaColeta) {
			HTML.Tag tag = this.getUltTag();
			HTML.Tag tag2 = this.getUltTag2();
			String url = this.getUtlUrl();
			String classe = null, classe2 = null;
			if (texto.indexOf("Veja")!=-1) {
				ligaColeta = false;
				return;
			}
			int i = this.getContadorLiAberto();
			if (i==1) {
				this.nomeCategoria = texto;
				int posicao = nomeCategoria.indexOf("(");
				if (posicao!=-1) {
					nomeCategoria = nomeCategoria.substring(0, posicao).trim();
				}
