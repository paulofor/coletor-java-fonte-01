...
public class DispositivoUsuarioMontador   implements MontadorDaoI { 
	...
	public  DCIObjetoDominio extraiRegistro( ResultadoLista resultadoLista  ,  DCIObjetoDominio entrada  ,  int pos )  throws  DaoException{
		DispositivoUsuario objeto;
		objeto = (DispositivoUsuario)entrada;
		  
	  	objeto.setIdDispositivoUsuario(resultadoLista.getInt(pos++));  
	  	objeto.setNumeroCelular(resultadoLista.getString(pos++));  
	  	objeto.setCodigoDispositivo(resultadoLista.getString(pos++));  
	  	objeto.setTipoAcesso(resultadoLista.getString(pos++));  
	  	objeto.setMelhorPath(resultadoLista.getString(pos++));  
	  	objeto.setTokenGcm(resultadoLista.getString(pos++));  
	  	objeto.setTokenGcmMonitor(resultadoLista.getString(pos++));  
	  	objeto.setMicroSd(resultadoLista.getInt(pos++));  
	  	objeto.setDataChamada(resultadoLista.getString(pos++));  
	  	objeto.setDataUltimoAcesso(resultadoLista.getString(pos++));  
	  	objeto.setDataCriacao(resultadoLista.getString(pos++));  
	  	objeto.setTemMudanca(resultadoLista.getLogicoConvertido(pos++));  
	  	objeto.setAtivo(resultadoLista.getLogicoConvertido(pos++));  
	  	objeto.setIdUsuarioRa(resultadoLista.getInt(pos++));  
	  	objeto.setIdAppProdutoU(resultadoLista.getInt(pos++));
      	return objeto;
	}
